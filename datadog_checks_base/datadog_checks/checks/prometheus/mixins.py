...
class PrometheusScraperMixin(object):
    ...
    def parse_metric_family(self, response):
        ...
        for metric in text_fd_to_metric_families(input_gen):
            ...
            #             (sample[0].endswith("_sum") or sample[0].endswith("_count")):
    ...
    def join_labels(self, metric):
        ...
        if self.label_joins:
            for sample in metric.samples:
                for label_name in sample[1].keys():
                    if label_name in self._watched_labels:
                        # Set this label value as active
                        if label_name not in self._active_label_mapping:
                            self._active_label_mapping[label_name] = {}
                        self._active_label_mapping[label_name][sample[1][label_name]] = True
                        # If mapping found add corresponding labels
                        try:
                            for label_tuple in self._label_mapping[label_name][sample[1][label_name]]:
                                sample[1][label_tuple[0]] = label_tuple[1]
                        except KeyError:
                            pass
        ...
        if metric.type in self.METRIC_TYPES:
            for sample in metric.samples:
                ...
                if metric.type == "counter":
                    ...
                    val = sample[2]
                    if self._is_value_valid(val):
                        if sample[0] in self.rate_metrics:
    ...
    def _get_hostname(self, hostname, sample):
        ...
        if hostname is None and self.label_to_hostname is not None:
            if self.label_to_hostname in sample[1]:
                return sample[1][self.label_to_hostname]
